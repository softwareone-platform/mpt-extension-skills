...
- Test dependencies (`pytest`, `pytest-cov`, `jsonschema`, `python-frontmatter`) are managed with `uv` in the `dev` dependency group of `pyproject.toml` and locked in `uv.lock`. Add or change them with `uv add --dev <pkg>` / `uv lock`.
...
- `tests/scripts/test_skill_structure.py` enforces skill structure: every `skills/<name>/SKILL.md` frontmatter validates against `schemas/skill_frontmatter.schema.json` (parsed with `python-frontmatter`, validated with `jsonschema`; `name` must match the directory), the body has the required sections from `standards/skills.md` (Purpose, Use When, Do Not Use When, Inputs/Prerequisites, Workflow, Guardrails, Expected Outcome), and `agents/openai.yaml` declares its three interface fields — closing the gap where a malformed frontmatter, missing section, or missing/empty adapter would still pass the token-budget check.
- `tests/scripts/test_docs_catalog_sync.py` keeps the shared-doc catalogs in sync: every `standards/*.md` and `knowledge/*.md` file must be linked from both `AGENTS.md` (agent index) and `README.md` (human index), so a doc can never exist on disk and in one index but be invisible in the other.
