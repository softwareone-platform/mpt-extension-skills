...
- Dashboard issue policy:
  - `fixVersions`: `v6` and `hotfix`
  - `Environment`: `prod`
  - `Keywords`: include `dashboard`
  - `HitCount`: use the finding `failures_count`
  - description/comment: include `operation_id` and `any_message` or stack trace in a Jira expand containing a code block
