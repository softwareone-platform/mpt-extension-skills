...
- Prefer MCP for writes to the Sprint field: `acli` has no command to move issues into a sprint, so setting Sprint requires MCP (or the Agile REST move-to-sprint endpoint). `acli` can set other custom fields (Team, Keywords, HitCount) at creation via `additionalAttributes`, but editing custom fields on existing issues through `acli` is unreliable — prefer MCP there too (see `standards/jira-fields.md` for the field IDs). When a required field cannot be set with the available interface, do not perform the write: stop before execution and report the blocker. Proceed with a partial write only after explicit user approval, and then verify every omitted field afterward.
...
python3 "${MPT_EXTENSION_SKILLS_HOME:-$HOME/.mpt-extension-skills}/current/skills/mpt-ext-tool-jira-workitem-ops/scripts/render_adf_with_attribution.py" --text "Update details" > /tmp/description_adf.json
...
- Render description or comment ADF with the bundled `render_adf_with_attribution.py` (see Bundled Resources for its runtime path).
...
- `scripts/render_adf_with_attribution.py`
  - Runtime path: `${MPT_EXTENSION_SKILLS_HOME:-$HOME/.mpt-extension-skills}/current/skills/mpt-ext-tool-jira-workitem-ops/scripts/render_adf_with_attribution.py`
