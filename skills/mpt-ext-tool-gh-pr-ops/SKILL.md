...
- Build the complete final description, including the `🤖 AI-generated PR — Please review carefully.` warning line when the content is AI-generated, before creating the PR. Set it in the single create call (`gh pr create --body-file`). Never open the PR with an empty or placeholder body and add the description or warning line in a later `gh pr edit`: automated reviewers such as CodeRabbit review on the `opened` event and would read the stale description.
- If the final description cannot be set at creation time, create the PR as a draft, set the complete description while it is still a draft, and only then mark it ready with `gh pr ready`. Reviewers configured with `drafts: false` do not review drafts, which avoids reviewing a stale description.
...
- Never open a PR with an empty or placeholder description and then add the required description or `🤖 AI-generated PR — Please review carefully.` warning line in a later edit; the complete description must be present in the initial create call, or the PR must be created as a draft and only marked ready after the description is finalized.
