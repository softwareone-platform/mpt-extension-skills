...
- When a fix was applied, post an explicit confirmation reply that the comment was addressed.
...
- Never mark a comment as handled silently; after applying a fix, always leave an explicit follow-up reply in the same thread.
